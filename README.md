...
- タスクの proposal 一覧: `npx opencode-orchestrator list --task <task-key> --proposals`
- proposal のクリア: `npx opencode-orchestrator clear --task <task-key> --proposals -y`
...
- Planner (`orch-planner`)
  ...
  - acceptance-index と spec.md の作成・更新は Refiner に委譲し、
    自身は主に Spec-Checker / Preflight-CLI の結果をまとめて「どのコマンドが必須で、どの程度環境が
    整っているか」を人間に説明します。

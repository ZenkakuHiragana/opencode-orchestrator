...
1. 実行フェーズでは計画通りにタスクを選定し、すべての受け入れ条件が達成されるまでエージェントを稼働させます。
   ...
   - `npx opencode-orchestrator --task task-name`
...
flowchart LR
  ...
  subgraph Execution["実行フェーズ"]
    direction TB
    Dev2{{"開発者"}} --"シェルスクリプト実行:<br/>npx opencode-orchestraor loop --task task-name"-->
...
1. (必要に応じて) Todo-Writer ステップ: `opencode run --command orch-plan ...`
2. Executor ステップ: `opencode run --command orch-exec ...`
3. Auditor ステップ: `opencode run --command orch-audit --format json ...`

a. Auditor が `done: true` を返した時点でループ終了
b. `--commit` 指定時は、完了後に追加の executor ステップを使って `autocommit` ツール経由のコミットを依頼
...
- 状態: `$(getOrchestratorBaseDir)/my-task/state`
  ...
  - `status.json` : Executor / Auditor の進捗スナップショットと、Todo-Writer 向けの正規化された再計画要求 (`replan_request`)
...
主なエージェントと役割は `AGENTS.md` に詳細がありますが、概要だけまとめます。
